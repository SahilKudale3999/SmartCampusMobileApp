      ...
      <div className="relative overflow-hidden animated-navy-banner rounded-3xl p-8 shadow-2xl shadow-blue-950/25 text-white">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div>
            ...
            <h1 className="text-3xl font-black tracking-tight text-white">Manage Students</h1>
            <p className="text-blue-200/80 text-sm mt-1 max-w-xl">
              Monitor student accounts, track enrollments, and manage campus registry records seamlessly.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="bg-white/10 border border-white/10 rounded-2xl px-4 py-3 backdrop-blur-md flex items-center gap-3">
              ...
              <div>
                <span className="block text-[11px] text-blue-200/70 font-medium">Total Students</span>
                <span className="text-lg font-bold text-white">{totalStudentsCount}</span>
              </div>
            </div>
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-200/80 overflow-hidden">
          <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">Active student accounts and enrollment status</p>
            </div>
          ...
          <StudentTable students={students} onChanged={fetchStudents} />
        </div>
      )}

      {/* Modal Container */}
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Add Student Record">
        <AddStudentForm onSuccess={handleAdded} />
      </Modal>
    </div>
  );
}